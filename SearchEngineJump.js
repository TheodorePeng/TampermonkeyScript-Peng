...
// @namespace      https://github.com/PengCode/searchEngineJump
...
(function () {
    ...
    console.log("脚本: 搜索引擎快捷跳转 --- 开始执行 --- Modified by Peng")

    function isYouTubeSearchPage(currentUrl) {
        return /^https?:\/\/www\.youtube\.com\/results/.test(currentUrl || window.location.href);
    };

    function getYouTubeSearchInput() {
        return document.querySelector('input[name="search_query"]')
            || document.querySelector('yt-searchbox input')
            || document.querySelector('input#search');
    };

    function isGoogleSearchPage(currentUrl) {
        return /^https?:\/\/www\.google(?:\.[A-z]{2,3}){1,2}\/search(?:[?#]|$)/.test(currentUrl || window.location.href);
    };
    ...
    function getGoogleSearchKeyword() {
        var input = getGoogleSearchInput();
        return input ? input.value : "";
    };

    function getGoogleToolbarTarget() {
        return document.querySelector('.Gcxb4e')
            || document.querySelector('#before-appbar')
            || document.querySelector('#appbar')
            || document.querySelector('#slim_appbar');
    };

    var trustedHtmlPolicy = null;
    function getTrustedHTML(html) {
        if (!window.trustedTypes || typeof html != "string") return html;

        if (!trustedHtmlPolicy) {
            try {
                trustedHtmlPolicy = window.trustedTypes.createPolicy('searchEngineJump', {
                    createHTML: function (value) {
                        return value;
                    }
                });
            } catch (error) {
                trustedHtmlPolicy = window.trustedTypes.createPolicy('searchEngineJump-' + Date.now(), {
                    createHTML: function (value) {
                        return value;
                    }
                });
    ...
    function setElementHTML(element, html) {
        element.innerHTML = getTrustedHTML(html);
    };
    ...
    function iqxinstart() {
        ...
        var rules = [
            ...
            },
            {
                ...
                insertIntoDoc: {
                    target: function () {
                        return document.querySelector('#appbar') || getGoogleToolbarTarget();
                    },
                    keyword: function () {
                        return getGoogleSearchKeyword();
                    },
            ...
            },
            {
                ...
                insertIntoDoc: {
                    keyword: function () {
                        var input = getYouTubeSearchInput();
                        return input ? input.value : "";
                    },
        ...
        function mousedownhandler(e) {
            ...
            if (typeof iInput == 'function') {
                ...
                if (iInput.nodeName == 'INPUT' || iInput.nodeName == 'TEXTAREA') {
                    value = iInput.value;
                } else {
        ...
        if (!matchedRule || !matchedRule.enabled) return false;
        ...
        SEJsetting.prototype = {
            ...
            addContent: function () {
                ...
                for (let i = 0; i < detailsLength; i++) {
                    ...
                    setElementHTML(oDivTitle, '<span class="iqxin-pointer-events">' + details[j][0] + '</span>' +
                    ...
                    setElementHTML(oDivCon, oDivConStr);
                ...
                setElementHTML(btnEle2, btnStr2);
                ...
                var btnStr = "<div class='btnEleLayer'>" +
                    ...
                    "<span class='feedback' title='问题反馈'><a target='_blank' href='#'>Feedback</a></span>" +
                ...
                nSearchList.textContent = "增加新的搜索列表";
            ...
            addEditBox: function (e) {
                ...
                setElementHTML(newDiv, innerHTML);
            ...
            addTitleEditBox: function (e) {
                ...
                if (flag && flag.parentNode == element) {
                    setElementHTML(element, element.firstChild.value ? element.firstChild.value : "空");
                    element.classList.add("iqxin-pointer-events");
                } else {
                    //  存在编辑的标题,但与点击的不是同一个节点
                    if (flag) {
                        setElementHTML(flag.parentNode, flag.parentNode.firstChild.value);
                    }
                    ...
                    newobj.onkeydown = function (e) {
                        if ((e.keyCode || e.which) == 13) {
                            setElementHTML(element, this.value ? this.value : oldhtml);
                        } else if ((e.keyCode || e.which) == 27) {
                            setElementHTML(element, oldhtml);
                        }
            ...
            addTitleEditBoxRemove: function () {
                ...
                if (odiv) {
                    setElementHTML(odiv.parentNode, odiv.value ? odiv.value : "空");
                }
            },
            ...
            setBtnOpacityFun: function () {
                if (~window.navigator.userAgent.indexOf("Chrome")) {
                    ...
                    if (getSettingData.setBtnOpacity < 0) {
                        document.querySelector(".iqxin-setBtnOpacityRangeValue").textContent = odomV.toString().padEnd(4, "0");
                        odom.style.background = "-webkit-linear-gradient(left,#3ABDC1,#83e7ea) no-repeat, #fff";
                    } else {
                        document.querySelector(".iqxin-setBtnOpacityRangeValue").textContent = "禁用";
            ...
            rangeChange: function (bool) {
                ...
                if (getSettingData.setBtnOpacity < 0) {
                    ...
                    document.querySelector(".iqxin-setBtnOpacityRangeValue").textContent = valueStr;
                    getSettingData.setBtnOpacity = odom.value;
                }
            },
    ...
    var youtubeReinjectTimer = null;
    var youtubeReadyObserver = null;
    var googleReinjectTimer = null;
    var googleReadyObserver = null;

    function reinjectYouTubeIfReady(forceRebuild) {
        if (!isYouTubeSearchPage()) return false;

        var target = document.querySelector('#page-manager');
        var keyword = getYouTubeSearchInput();
        if (!target || !keyword) return false;

        var sejContainer = document.querySelector('#sej-container');
        if (sejContainer && !forceRebuild) return true;
    ...
    function scheduleYouTubeReinject(forceRebuild) {
        clearTimeout(youtubeReinjectTimer);
        youtubeReinjectTimer = setTimeout(function () {
            var injected = reinjectYouTubeIfReady(forceRebuild);
            if (injected && youtubeReadyObserver) {
                youtubeReadyObserver.disconnect();
                youtubeReadyObserver = null;
            }
        }, forceRebuild ? 0 : 120);
    };

    function watchYouTubeSearchPageReady() {
        if (!isYouTubeSearchPage()) return;

        var root = document.querySelector('ytd-app') || document.body;
        if (!root) return;

        if (youtubeReadyObserver) {
            youtubeReadyObserver.disconnect();
        }

        youtubeReadyObserver = new (window.MutationObserver || window.WebKitMutationObserver)(function () {
            scheduleYouTubeReinject(false);
        });
        youtubeReadyObserver.observe(root, { childList: true, subtree: true });
        scheduleYouTubeReinject(false);
    };

    function initYouTubeSej() {
        watchYouTubeSearchPageReady();

        document.addEventListener('yt-navigate-finish', function () {
            clearSejUi();
            if (!isYouTubeSearchPage()) return;
            watchYouTubeSearchPageReady();
            scheduleYouTubeReinject(true);
        }, true);
    };

    function reinjectGoogleIfReady(forceRebuild) {
        if (!isGoogleSearchPage()) return false;

        var target = getGoogleToolbarTarget();
        var keyword = getGoogleSearchInput();
        if (!target || !keyword) return false;
    ...
    function scheduleGoogleReinject(forceRebuild) {
        clearTimeout(googleReinjectTimer);
        googleReinjectTimer = setTimeout(function () {
            reinjectGoogleIfReady(forceRebuild);
        }, forceRebuild ? 0 : 120);
    };

    function watchGoogleSearchPageReady() {
        if (!isGoogleSearchPage()) return;

        var root = document.body || document.documentElement;
        if (!root) return;

        if (googleReadyObserver) {
            googleReadyObserver.disconnect();
        }

        googleReadyObserver = new (window.MutationObserver || window.WebKitMutationObserver)(function () {
            if (!document.querySelector('#sej-container')) {
                scheduleGoogleReinject(false);
            }
        });
        googleReadyObserver.observe(root, { childList: true, subtree: true });
        scheduleGoogleReinject(false);
    };

    function initGoogleSej() {
        watchGoogleSearchPageReady();

        window.addEventListener('popstate', function () {
            clearSejUi();
            if (!isGoogleSearchPage()) return;
            watchGoogleSearchPageReady();
            scheduleGoogleReinject(true);
        }, true);
    };
    ...
    if (isYouTubeSearchPage()) {
        ...
        setTimeout(function () {
            // console.log("延时运行");
            var sejSpan = document.querySelector('#sej-container');
            if (sejSpan) {
                return;
            } else {
                iqxinstart();
