...
| 脚本 | 功能简介 | 适用站点 |
|------|---------|---------|
| **[ChatGPTAutoReadAloud.user.js](ChatGPTAutoReadAloud.user.js)** | 配合 ChatGPT Audio Controls 扩展，在新回答完成后自动触发原生 Read Aloud；兼容新 Chat 从临时 URL 切换到正式会话 URL 的首轮回答，并提供可拖拽开关、扩展 Logo 隐藏设置和实验性裸方向键 seek（受上游控件行为影响，可能无效，建议保持关闭） | `chatgpt.com/*` |
